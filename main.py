...
def get_data(ticker):
    ...
    if pe_current < pe_low_hist: pe_low_hist = pe_current * PE_LOW_HIST_FALLBACK_MULT
...
if __name__ == "__main__":
    ...
    for i, d in enumerate(ALL_DATA):
        ...
        sheet = workbook.add_worksheet(ticker.split('.')[0])
        ...
        f_calc_pemid = f'IF(ISNUMBER(SEARCH("Static",{cell_pe})), 30, IF(ISNUMBER(SEARCH("Custom",{cell_pe})), 20, {f_pe_rt}))'
        f_calc_pestep = f'IF(ISNUMBER(SEARCH("Static",{cell_pe})), 5, IF(ISNUMBER(SEARCH("Custom",{cell_pe})), 0, ({ref_peh}-{ref_pel})/6))'
        ...
        def draw_grid_formulas(start_row, table_type):
            ...
            for j in range(7):
                ...
                form = f'={f_calc_pemid} + ({f_calc_pestep}*{offset})'
